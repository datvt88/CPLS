...
export async function GET() {
  const url = buildVNDirectUrl('/v4/top_stocks', {
    q: 'index:VNIndex~lastPrice:gte:6~nmVolumeAvgCr20D:gte:100000~priceChgPctCr1D:gt:0',
    size: '10',
    sort: 'priceChgPctCr1D'
  })

  // Fallback data for when API is unavailable
  const fallbackData: VNDirectResponse<TopGainerStock> = {
    data: [
      {
        code: 'HPG',
        index: 'VNINDEX',
  ...
  const rawData = await fetchVNDirectWithFallback<VNDirectResponse<any>>(
    url,
    fallbackData,
    { next: { revalidate: 3 } } // Revalidate every 3 seconds
  )

  // Normalize the data to ensure type safety
